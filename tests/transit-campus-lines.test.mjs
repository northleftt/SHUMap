...
test("campus-lines 响应带 dayType，标签只读校历，与班次归属互不影响", async () => {
  const db = database();
  // 2026-08-21 落在 0035 种子的暑假区间（2026-08-03 – 2026-09-13）→ summer_break。
  // 日型标签是校历的固有属性：服务日历（day_type、星期勾选、例外）只决定班次归属。
  const fallback = await callJson(db, "from=campus_baoshan&to=campus_jiading&date=2026-08-21");
  assert.equal(fallback.dayType, "summer_break");

  // 把 cal_a 的 day_type 改成 holiday：标签**不**跟着变（校历为准），班次归属也不变。
  db.prepare("update service_calendars set day_type='holiday' where id='cal_a'").run();
  const after = await callJson(db, "from=campus_baoshan&to=campus_jiading&date=2026-08-21");
  assert.equal(after.dayType, "summer_break", "服务日历的 day_type 不再影响标签");
  assert.deepEqual(
    after.lines.flatMap((line) => line.journeys.map((journey) => journey.tripId)).sort(),
    fallback.lines.flatMap((line) => line.journeys.map((journey) => journey.tripId)).sort(),
    "改 day_type 不该改变班次归属",
  );

  // 周六（saturday=0，日历没命中）：标签仍按校历（8/22 同在暑假），班次为空
  const saturday = await callJson(db, "from=campus_baoshan&to=campus_jiading&date=2026-08-22");
  assert.equal(saturday.dayType, "summer_break");

  // added 例外日（2026-08-23 周日加开）：班次回来，标签仍按校历
  const added = await callJson(db, "from=campus_baoshan&to=campus_jiading&date=2026-08-23");
  assert.equal(added.dayType, "summer_break");
  assert.ok(added.lines.flatMap((line) => line.journeys).length > 0, "added 例外日恢复班次");

  // removed 例外日（2026-08-28 周五停运）：班次为空，标签仍按校历
  const removed = await callJson(db, "from=campus_baoshan&to=campus_jiading&date=2026-08-28");

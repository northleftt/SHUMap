...
BEGIN
  select raise(abort,'place kind must belong to an active map filter')
    WHERE not exists (
    select 1
...
when new.lifecycle_status <> 'retired'
BEGIN
  select raise(abort,'facility type must be enabled and belong to an active map filter')
    WHERE not exists (
    select 1
...
when new.lifecycle_status <> 'retired'
BEGIN
  select raise(abort,'merchants must belong to an active map filter')
    WHERE not exists (
    select 1
      from map_filter_members m
      join map_filter_categories c on c.id=m.category_id and c.active=1
...
BEGIN
  select raise(abort,'active facility type must belong to an active map filter')
    WHERE not exists (
    select 1
      from map_filter_members m
      join map_filter_categories c on c.id=m.category_id and c.active=1
...
BEGIN
  select raise(abort,'map filter with live members cannot be deactivated')
    WHERE exists (
    select 1
      from map_filter_members m
...
BEGIN
  select raise(abort,'live map filter member cannot move to an inactive map filter')
    WHERE not exists (
...
BEGIN
  select raise(abort,'live map filter member cannot be deleted')
    WHERE exists(select 1 from places p where p.kind_id=old.place_kind_id and p.lifecycle_status<>'retired')
    or exists(
      select 1 from facility_types t
      left join facility_instances f on f.facility_type_id=t.id and f.lifecycle_status<>'retired'
      where t.id=old.facility_type_id and (t.status='active' or f.id is not null)

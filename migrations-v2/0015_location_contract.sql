...
begin
  ...
  then raise(abort,'navigation target must be a valid GCJ02 Point') end;
end;
...
begin
  ...
  ) then raise(abort,'entity location role must match anchor role') end;
end;
...
begin
  select case when new.entity_type<>'place' or not exists (
    select 1 from location_anchors anchor
     where anchor.id=new.anchor_id and anchor.building_place_id=new.entity_id
  ) then raise(abort,'footprint binding must belong to its anchor building') end;
end;
...
begin
  ...
  ) then raise(abort,'map feature already has an active footprint binding') end;
end;
...
begin
  ...
  then raise(abort,'footprint anchor must reference a canonical campus building feature') end;
end;
...
begin
  select case when
    new.feature_kind<>'building_footprint'
  ...
  then raise(abort,'active footprint feature must preserve its canonical contract') end;
end;

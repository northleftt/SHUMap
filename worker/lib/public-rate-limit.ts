 ...
 * 而从一台与开发机无关的第三方主机请求容器公网域名，`/healthz` 仍然回 200 ——
 * 关闭开关并没有真的收掉入口。所以**不要把「公网已关」当成安全前提**，
 * 它既不由本仓库保证，实测也不成立。
 *
 * 复测只能用外部出口（第三方主机 / 手机流量）。开发机上的 curl 一律不可信：本机
 * Clash TUN 会把到该地址的流量整条截走（`route get` 显示出口是 utun 而非 en0），
 * `--noproxy '*'`、`--resolve` 真实 IP、甚至 `--interface en0` 都绕不开，
 * 测出来的 200 只说明代理出口节点能到。详见 HANDOFF「云托管代理共享限流桶」。
 *

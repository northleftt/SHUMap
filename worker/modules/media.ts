...
/**
 * POST /api/public/media — 匿名照片上传（一段式，raw body）。
 *
 ...
 */
export async function createPublicMediaUpload(request: Request, env: Env): Promise<Response> {
  ...
  ).bind(mediaId, objectKey, contentType, bytes.byteLength, digest, isoNow()).run();

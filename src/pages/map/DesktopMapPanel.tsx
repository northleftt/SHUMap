...
export function DesktopMapPanel({ state }: { state: MapState }) {
  ...
  return (
    <aside className="flex w-[280px] shrink-0 xl:w-[360px] flex-col border-r border-line bg-surface">
      ...
      <div className="shrink-0 border-b border-line px-3 py-3"><FeatureFeedback page="search" prompt="你觉得搜索好用吗？" /></div>
...
export function PoiMapCard({
  ...
  const isCanteen = building.entityType === "building" && building.kindId === "canteen";
  return (
    <div className="absolute bottom-6 left-6 z-30 w-[calc(100%-3rem)] max-w-[300px] rounded-2xl bg-surface p-4 shadow-floating">
      ...
      <button type="button" className="mt-3 w-full rounded-xl bg-primary-container px-3 py-2.5 text-left text-body font-medium text-primary" onClick={onOpenMerchants}>查看地点详情 ›</button>
      {/* 楼内商户入口（release manifest merchants 归到本楼） */}
      {isCanteen || merchantCount > 0 ? (
        <button
          type="button"
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-body font-medium text-ink">{isCanteen ? "楼层与档口" : `楼内商户 ${merchantCount} 家`}</span>
            <span className="mt-0.5 block truncate text-aux text-sub">
              {isCanteen ? "查看供餐详情 ›" : building.merchants.map((merchant) => merchant.name).join("、")}
            </span>
          </span>
      ...
      <div className="mt-3 flex gap-2">
        ...
        {building.entityType === "building" ? (
          <button
            ...
            onClick={() => navigate(`/places/${building.entityId}/${isCanteen ? "dining" : "floors"}`)}
          >
            {isCanteen ? "供餐详情" : "楼层设施"}
          </button>
        ) : null}

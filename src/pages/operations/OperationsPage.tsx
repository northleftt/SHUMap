...
function eventTypeLabel(eventType: string): string {
  return EVENT_TYPE_LABELS[eventType] ?? "通知";
}
...
export function OperationsPage() {
  ...
  const { activeEvents, endedEvents, status } = useOperations();
  const { state: placeState } = useAsyncData((signal) => getPlace(placeId, signal), [placeId]);
  const now = useNow(60_000);

  const place = placeState.status === "ready" && placeState.data ? placeState.data.place : null;
  const facilityIds = new Set(
    placeState.status === "ready" && placeState.data ? placeState.data.facilities.map((f) => f.id) : [],
  );

  // targets 命中本楼或楼内设施；无 targets 数据（旧 release）时全部展示
  ...
  const active = activeEvents.filter(matches);
  const ended = endedEvents.filter(matches);
                      ...
                      <article key={event.id} className="rounded-2xl bg-surface p-4 shadow-card">
                        <div className="flex items-center justify-between">
                          <div className={`flex items-center gap-1.5 text-aux font-medium ${typeColor}`}>
                            <TypeIcon size={15} />
                            {eventTypeLabel(event.eventType)} · {SEVERITY_LABELS[severity]}
                          </div>
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-0.5 text-aux text-sub">
                          ...
                          {OPERATIONAL_STATUS_LABELS[event.operationalStatus] ?? "已结束"}
                        </div>
                      </div>
                    </div>
                  ))}

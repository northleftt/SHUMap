...
//
// 只读：全部是 SELECT，不写库、不碰 R2。给 0032/0033 迁移上线前后的盘点用。
//
...
console.log(`迁移状态: 0032_floor_plan_image ${applied.includes("0032_floor_plan_image.sql") ? "已应用" : "未应用"}，0033_floor_svg_purge ${applied.includes("0033_floor_svg_purge.sql") ? "已应用（楼层 SVG 已清除）" : "未应用"}`);
...
// ── indoor_spaces：已无管理入口，表与引用保留 ────────────────────────────
summary.indoorSpaces = section(
  "indoor_spaces（楼内空间，保留但不再能新建）",
  query(

...
// 搬运自 shared/geo-transform.mjs（逻辑逐行保留，类型补自 shared/geo-transform.d.mts）。
// 约定：库内所有地理坐标一律 gcj02；wgs84 先经 wgs84ToGcj02 转换再进仿射。
...
export function fitGeoTransform(points: GeoControlPoint[]): GeoTransform {
  ...
  const det = sll * saa - sla * sla;
  if (Math.abs(det) < 1e-20) throw new Error("fitGeoTransform control points are degenerate");
  const fittedA = (slx * saa - sax * sla) / det;
  const fittedB = (sax * sll - slx * sla) / det;
  const fittedD = (sly * saa - say * sla) / det;
  const fittedE = (say * sll - sly * sla) / det;
  return {
    a: fittedA,
    b: fittedB,

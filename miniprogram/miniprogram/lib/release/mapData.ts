...
// 搬运自 Web 端 src/lib/release/mapData.ts（逻辑原样）；网络层换成小程序 api 通道：
// getCurrentRelease → apiGet + parseReleaseManifest，fetchMapAssetSvg → apiGetText。
// AbortSignal 在小程序里没有对应物，直接去掉（原签名只有 Web 端内部使用）。
...
function detailOf(place: ReleasePlace): PoiDetailData {
  ...
  const media = detail.media.map((item, index) => {
    ...
    return {
      ...
      ...(typeof row.floorLevelCode === "string" ? { floorLevelCode: row.floorLevelCode } : {}),
    };
  });
...
function primaryPointLocations(manifest: ReleaseManifest): Map<string, ReleaseLocation> {
  ...
  // 仍可读，只作低优先级兜底。与 src/lib/release/mapData.ts 保持一致。
  ...
  return manifest.places
    ...
    .map((place) => {
      ...
      return {
        ...
        navigationPoint: nav ? navPoint(nav, place.displayName) : null,
...
export function buildMapPointPois(
  ...
  for (const facility of manifest.facilities) {
    ...
    result.push({
      ...
      navigationPoint: nav ? navPoint(nav, facility.displayName) : null,
      facilities: [],
      merchants: [],
  ...
  for (const merchant of manifest.merchants) {
    ...
    result.push({
      ...
      navigationPoint: nav ? navPoint(nav, merchant.displayName) : null,
      facilities: [],
      merchants: [],
      facilityOperationalStatus: null,
      visibility: DEFAULT_POINT_VISIBILITY,
  ...
  // 与 src/lib/release/mapData.ts 保持一致。
  //
  ...
  for (const stop of manifest.transit.stops) {
    ...
    result.push({
      ...
      navigationPoint: nav ? navPoint(nav, stop.name) : null,
      facilities: [],
      merchants: [],
      facilityOperationalStatus: null,
      visibility: DEFAULT_POINT_VISIBILITY,
...
/**
 * 校区图版本（floor_id === null），含完整性/唯一性校验。
 * loadCampuses 与带缓存的 loader（./loader.ts）共用这份校验，保证两条路一致。
 */
export function campusMapVersions(manifest: ReleaseManifest): [ReleaseMapVersion, ...ReleaseMapVersion[]] {
  const campusMaps = manifest.maps.filter((map) => map.floor_id === null);
  const [first, ...rest] = campusMaps;
  if (!first) throw contractError("release has no campus maps");
  ...
  for (const map of campusMaps) {
    ...
    if (keys.has(key)) throw contractError(`duplicate campus map for ${key}`);
    keys.add(key);
  }
  return [first, ...rest];
}

/** 校区图版本 + SVG 原文 → CampusConfig（视口参数来自 CAMPUS_DISPLAY）。 */
export function campusConfigFromMap(map: ReleaseMapVersion, svgRaw: string): CampusConfig {
  // campusMapVersions 已做过完整性校验；这里防御性地再校验一次，允许独立调用。
  if (!map.campus_id || !map.campusCode || !map.campusName) {
    throw contractError(`campus map ${map.id} has incomplete campus metadata`);
  }
  const key = campusKey(map.campusCode);
...
/** GET /api/public/maps/:mapVersionId/asset — SVG 原文（非 JSON，走 apiGetText）。 */
function fetchMapAssetSvg(mapVersionId: string): Promise<string> {
  return apiGetText(`/api/public/maps/${encodeURIComponent(mapVersionId)}/asset`);
}

async function loadCampuses(manifest: ReleaseManifest): Promise<[CampusConfig, ...CampusConfig[]]> {
  const configs = await Promise.all(
    campusMapVersions(manifest).map(async (map) => campusConfigFromMap(map, await fetchMapAssetSvg(map.id))),
  );
  return configs as [CampusConfig, ...CampusConfig[]];
}
...
/** GET /api/public/releases/current — 活动 release 的冻结工件。 */
async function getCurrentRelease(): Promise<ReleaseManifest> {
  const value = await apiGet<unknown>("/api/public/releases/current");
...
export async function loadRelease(): Promise<LoadedRelease> {
  const manifest = await getCurrentRelease();
  const campuses = await loadCampuses(manifest);

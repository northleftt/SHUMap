// SVG 几何解析。搬运自 shared/svg-geometry.mjs（逻辑逐行保留），
// 类型补自 shared/svg-geometry.d.mts。.mjs 一律转 .ts——小程序编译器只打包源码模块。
...
function requiredNumbers(value: unknown, count: number, context: string): number[] {
...
function pathArguments(code: string, args: number[], stride: number, minimum = stride): void {
  if (args.length < minimum || args.length % stride !== 0) {
    throw new Error(`SVG path command ${code} has an invalid argument count`);
  }
}

function parsePath(value: string): ParsedShape {
  ...
  for (const chunk of chunks) {
    ...
    if (["C", "S", "Q", "T", "A"].includes(upper)) {
      const stride = { C: 6, S: 4, Q: 4, T: 2, A: 7 }[upper] as number;
...
function finiteAttribute(attrs: string, name: string, defaultValue?: number): number {
...
function parseShape(tag: string, attrs: string): ParsedShape {
  if (tag === "path") {
...
function ringArea(ring: SvgPoint[]): number {
  let sum = 0;
...
function onSegment(point: SvgPoint, start: SvgPoint, end: SvgPoint): boolean {
...
function ringContains(ring: SvgPoint[], point: SvgPoint): boolean {
  let inside = false;
...
function polygonGeometry(subPaths: SvgSubPath[]): Record<string, unknown> | null {
  const nodes: RingNode[] = subPaths
...
function geometryOf(subPaths: SvgSubPath[]): Record<string, unknown> | null {
...
function bboxOf(subPaths: SvgSubPath[]): [number, number, number, number] | null {
...
function stableKey(sourceElementId: string): string | null {
...
export function parseSvgFeatures(svg: string): ParsedSvgFeature[] {
  parseSvgViewBox(svg);
  const features: ParsedSvgFeature[] = [];
